...
namespace RentCar.Application.Reports.Reservation
{
    public sealed class ReservationInvoiceReport : IReport
    {
        private readonly RentCarDbContext _context;

        public string Code => "RESERVATION_INVOICE";

        public ReservationInvoiceReport(RentCarDbContext context)
        {
            _context = context;
        }
            ...
            CancellationToken ct)
        {
            ...
            var ds = new DataSet("ReservationInvoice");
        ...
        {
            var r = await _context.Reservations.FirstAsync(x => x.Id == reservationId, ct);
            ...
            t.Rows.Add(r.Id, r.PickupDate, r.DropoffDate, r.TotalPrice, r.CreatedAt);
            return t;
        }

        private async Task<DataTable> BuildBusiness(int reservationId, CancellationToken ct)
        {
            var b = await _context.Reservations.Where(r => r.Id == reservationId)
                .Select(r => r.Business)
                .FirstAsync(ct);
        ...
        {
            var c = await _context.Reservations.Where(r => r.Id == reservationId)
                .Select(r => r.Customer)
                .FirstAsync(ct);
        ...
        {
            var car = await _context.Reservations.Where(r => r.Id == reservationId)
                .Select(r => r.Car)
                .FirstAsync(ct);
